...
### 5.7 The Fancy P&L Ladder

The **P&L Ladder** is a table that shows the user their projected profit or loss at **every possible run value**. It's the most important tool for understanding fancy market positions.

#### How the Ladder Is Built

```
1. Collect all open bets for this fancy runner (same market + selection)
2. Find the MIN and MAX run lines across all bets
3. Generate a range: (MIN − 3) to (MAX + 3)
4. For each run value in that range:
     → Simulate the P&L as if the actual result was that value
     → Sum across all bets
5. Display as a two-column table: Run | Amount
```

#### Ladder Simulation Logic

For each simulated run value, apply this rule to every bet:

```
For a YES (BACK) bet with run line R and volume V:
    If run_value ≥ R  →  P&L = +(stake × volume) / 100     (YES wins)
    If run_value < R  →  P&L = −stake                       (YES loses)

For a NO (LAY) bet with run line R and volume V:
    If run_value < R  →  P&L = +stake                       (NO wins)
    If run_value ≥ R  →  P&L = −(stake × volume) / 100      (NO loses)
```

Sum the P&L of all bets at each run value. That's the ladder row.

#### Worked Example — Full Ladder

> **Open bets on "1st Innings Runs":**
>
> | # | Type | Run Line | Volume | Stake |
> |---|------|----------|--------|-------|
> | 1 | YES (BACK) | 38 | 90 | ₹5,000 |
> | 2 | NO (LAY) | 40 | 80 | ₹4,000 |
> | 3 | YES (BACK) | 42 | 100 | ₹3,000 |

**Range**: MIN line = 38, MAX line = 42 → Ladder from **(38 − 3) = 35** to **(42 + 3) = 45**

For each run value, compute each bet's P&L:

| Run | Bet 1 (YES 38 @ 90) | Bet 2 (NO 40 @ 80) | Bet 3 (YES 42 @ 100) | **Net P&L** |
|-----|---------------------|---------------------|-----------------------|-------------|
| 35 | −₹5,000 *(< 38)* | +₹4,000 *(< 40)* | −₹3,000 *(< 42)* | **−₹4,000** 🔴 |
| 36 | −₹5,000 | +₹4,000 | −₹3,000 | **−₹4,000** 🔴 |
| 37 | −₹5,000 | +₹4,000 | −₹3,000 | **−₹4,000** 🔴 |
| **38** | **+₹4,500** *(≥ 38)* | +₹4,000 | −₹3,000 | **+₹5,500** 🟢 |
| 39 | +₹4,500 | +₹4,000 | −₹3,000 | **+₹5,500** 🟢 |
| **40** | +₹4,500 | **−₹3,200** *(≥ 40)* | −₹3,000 | **−₹1,700** 🔴 |
| 41 | +₹4,500 | −₹3,200 | −₹3,000 | **−₹1,700** 🔴 |
| **42** | +₹4,500 | −₹3,200 | **+₹3,000** *(≥ 42)* | **+₹4,300** 🟢 |
| 43 | +₹4,500 | −₹3,200 | +₹3,000 | **+₹4,300** 🟢 |
| 44 | +₹4,500 | −₹3,200 | +₹3,000 | **+₹4,300** 🟢 |
| 45 | +₹4,500 | −₹3,200 | +₹3,000 | **+₹4,300** 🟢 |

**Reading the ladder:**
- Rows with the **run line values (38, 40, 42)** are highlighted — this is where the P&L flips
- Below 38: all bets that need high scores lose → **−₹4,000**
- 38–39: Bet 1 flips to profit, total jumps to **+₹5,500**
- 40–41: Bet 2 (NO) starts losing → drops to **−₹1,700**
- 42+: Bet 3 (YES) kicks in → recovers to **+₹4,300**
- **Worst case from ladder = −₹4,000** → this is the exposure for this runner

#### Admin/Agent Perspective — The Inversion Rule

The platform is the counterparty to every user bet. This means:

```
Admin P&L at any run value = −(User P&L at that run value)
```

When displaying the ladder to an **admin or agent**, simply **negate every amount**:

| Run | User Sees | Admin Sees |
|-----|-----------|------------|
| 35 | −₹4,000 🔴 | +₹4,000 🟢 |
| 38 | +₹5,500 🟢 | −₹5,500 🔴 |
| 42 | +₹4,300 🟢 | −₹4,300 🔴 |
...
> [!NOTE]
> This inversion applies to the **display only** — the underlying calculation is identical. The admin's worst case is the user's best case, and vice versa.

#### Key Ladder Implementation Rules

1. **Range**: Always show `MIN(run_lines) − 3` to `MAX(run_lines) + 3` — this gives enough context above and below
2. **Highlight**: Mark rows where run value equals a bet's run line — these are the "flip points" where P&L changes
3. **P&L zones are flat between flip points**: Notice how the same net P&L repeats for consecutive rows between run lines (e.g., runs 35–37 all show −₹4,000). The P&L only changes at run line boundaries
4. **Exposure = abs(most negative row)**: The single worst row in the entire ladder determines the exposure locked for this runner
5. **Independent per runner**: Each fancy runner has its own ladder. Total fancy exposure = sum of each runner's worst case
